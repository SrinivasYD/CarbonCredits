...
async function fetchAndSyncEvents() {
  try {
    ...
    const submittedEvents = await projectApprovalContract.queryFilter(
      "ProjectSubmitted"
    );
    ...
    const approvedEvents = await projectApprovalContract.queryFilter(
      "ProjectApproved"
    );
    ...
    const revokedEvents = await projectApprovalContract.queryFilter(
      "ProjectRevoked"
    );
...
function setupEventListeners() {
  ...
  projectApprovalContract.on("ProjectApproved", async (owner, approvalHash) => {
    console.log(
      `Project approved for ${owner}. Approval Hash: ${approvalHash}`
    );
    try {
      await Project.findOneAndUpdate(
        { owner },
