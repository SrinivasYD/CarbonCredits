...
import "../styles/Marketplace.css"; // CSS for card styling
...
const MarketplacePage = () => {
    ...
    try {
      ...
      for (let i = 0; i < totalSupply; i++) {
        try {
          ...
          // Load the image from the public folder using the process.env.PUBLIC_URL
          ...
          // If the connected account is the owner, add to owned list
          if (owner.toLowerCase() === accountAddress.toLowerCase()) {
            let isListed = false;

            // Try fetching listing details for the NFT
            try {
              const listing = await marketplace.listings(i);
              if (listing.price > 0 && !listing.isSold) {
                isListed = true;
              }
            } catch (error) {
              // NFT is not listed, skip the listing error
            }

            // Add the NFT to the owned list with listing status
            ownedNftsList.push({
              tokenId: i,
              isListed,
              imageUrl, // Load image from the public folder
            });
          } else {
            // Check if the NFT is listed for sale and exclude NFTs owned by the connected account
            try {
              const listing = await marketplace.listings(i);
              if (
                listing.price > 0 &&
  ...
  return (
    <div className="marketplace-container">
      ...
      {/* Section 1: Listed NFTs for Sale in a Carousel */}
      ...
      {/* Section 2: Owned NFTs with Option to List in a Carousel */}
      <div className="nft-carousel">
        <h2>Your NFTs</h2>
        {ownedNfts.length === 0 ? (
            ...
            {ownedNfts.map((nft) => (
              <div key={nft.tokenId} className="nft-card">
                ...
                <div className="nft-info">
                  <h3>NFT #{nft.tokenId}</h3>
                  {nft.isListed ? (
                    <p>This NFT is already listed for sale.</p>
                  ) : (
                    <div>
                      <input
                        type="text"
